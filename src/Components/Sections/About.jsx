...
import Leah from "./../../Assets/EmployeePhotos/Leah_adjusted.png";
        ...
        <Col xs={12} sm={6} md={4} lg={3} key={key}>
          <div className="about__image-container d-flex align-items-end rounded">
            <Image
              src={member.image}
              fluid
              className={`about__image d-block mx-auto`}
            />
          </div>
          <div className="about__email text-center mt-2 mb-3">
            <span className="ml-2 font-weight-bold">{member.name}</span>
            <small className="d-block font-weight-light font-italic">
              {member.title}
            </small>
            <a href={`mailto:${member.email}`}>
              <FontAwesomeIcon icon="envelope" color="#062348" />
            </a>
            {member.bio && (
              <FontAwesomeIcon
                icon="info-circle"
    ...
    <>
      ...
      {modalInfo && (
        <CustomModal show={show} handler={setShow}>
          ...
          <Image
            className="about-photo d-block mx-auto mb-2"
            src={modalInfo.image}
            fluid
            rounded
          />
          ...
          <div className="about__email text-center mt-2 mb-3">
            <FontAwesomeIcon icon="envelope" color="#062348" />
            <span className="ml-2 font-weight-bold">{modalInfo.email}</span>
          </div>
        </CustomModal>
      )}
    </>
